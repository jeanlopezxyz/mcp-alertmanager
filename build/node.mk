...
npm-copy-project-files: npm-copy-binaries ## Copy the project files to the main npm package and generate all package.json files
	...
	@echo '"description": "Model Context Protocol (MCP) server for Prometheus Alertmanager",' >> $(MAIN_PACKAGE_JSON)
	...
	@echo '"repository": {"type": "git", "url": "git+https://github.com/jeanlopezxyz/mcp-alertmanager.git"},' >> $(MAIN_PACKAGE_JSON)
	@echo '"keywords": ["mcp","alertmanager","prometheus","monitoring","model-context-protocol"],' >> $(MAIN_PACKAGE_JSON)
	@echo '"license": "MIT",' >> $(MAIN_PACKAGE_JSON)
	@echo '"bugs": {"url": "https://github.com/jeanlopezxyz/mcp-alertmanager/issues"},' >> $(MAIN_PACKAGE_JSON)
	@echo '"homepage": "https://github.com/jeanlopezxyz/mcp-alertmanager#readme",' >> $(MAIN_PACKAGE_JSON)
	@echo '"mcpName": "io.github.jeanlopezxyz/mcp-alertmanager"' >> $(MAIN_PACKAGE_JSON)
	@echo '}' >> $(MAIN_PACKAGE_JSON)
	$(foreach os,$(OSES),$(foreach arch,$(ARCHS), \
		...
		echo '"description": "Model Context Protocol (MCP) server for Prometheus Alertmanager",' >> $$OS_PACKAGE_JSON; \
		echo '"repository": {"type": "git", "url": "git+https://github.com/jeanlopezxyz/mcp-alertmanager.git"},' >> $$OS_PACKAGE_JSON; \
		OS="$(os)"; \
