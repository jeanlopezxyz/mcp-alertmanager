...
const GITHUB_REPO = 'jeanlopezxyz/mcp-alertmanager';
const JAR_NAME = 'mcp-alertmanager.jar';
const CACHE_DIR = path.join(require('os').homedir(), '.cache', 'mcp-alertmanager');
...
function showHelp() {
  console.log(`
mcp-alertmanager - MCP Server for Prometheus Alertmanager

USAGE:
  npx mcp-alertmanager [OPTIONS]
...
EXAMPLES:
  ...
  ALERTMANAGER_URL="http://alertmanager:9093" npx mcp-alertmanager

  # SSE mode on port 9082
  ALERTMANAGER_URL="http://alertmanager:9093" npx mcp-alertmanager --port 9082
...
async function getLatestRelease() {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      headers: { 'User-Agent': 'mcp-alertmanager' }
    };
...
function downloadFile(url, dest) {
  return new Promise((resolve, reject) => {
    ...
    const request = (url) => {
      https.get(url, { headers: { 'User-Agent': 'mcp-alertmanager' } }, (res) => {
...
async function getJar(verbose = true) {
  ...
  try {
    ...
    if (verbose) console.error(`Downloading mcp-alertmanager ${latestVersion}...`);
...
async function main() {
  ...
  // Default ALERTMANAGER_URL if not set
